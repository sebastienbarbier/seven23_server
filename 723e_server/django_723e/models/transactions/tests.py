...
from django_723e.models.transactions.models import Category, Transaction, Cheque, Change, Transaction2Change
import datetime
...
class AccountTest(TransactionTestCase):
    ...
    def test_CategoriesDelete(self):
        ...
        """
        trans1 = Transaction.objects.create(account=self.account,
                                            currency=self.euro,
                                            name="Shopping",
                                            amount=1,
                                            category=self.cat1)
    ...
    def test_CategoriesSum(self):
        ...
        # Second transaction
        trans2 = Transaction.objects.create(account=self.account,
                                            currency=self.euro,
    ...
    def test_Change_Transactions(self):
        ...
        trans2 = Transaction.objects.get(pk=trans2.pk)
    ...
    def test_Change_Transactions3(self):
        ...
        c2.delete()
        trans1 = Transaction.objects.get(pk=trans1.pk)
    ...
    def test_Change_Delete_Transactions(self):
        c1 = Change.objects.create(account=self.account,
                                   ...
                                   new_amount=40,
                                   new_currency=self.chf)
        t1 = Transaction.objects.create(account=self.account,
                                            currency=self.chf,
                                            date=datetime.datetime.today() - datetime.timedelta(days=2),
                                            name="Achat 2 en suisse",
                                            amount=60)
        c2 = Change.objects.create(account=self.account,
                                   ...
                                   new_amount=40,
                                   new_currency=self.chf)
        t2 = Transaction.objects.create(account=self.account,
                                            currency=self.chf,
                                            date=datetime.datetime.today() - datetime.timedelta(days=2),
                                            name="Achat 2 en suisse",
                                            amount=60)
        c3 = Change.objects.create(account=self.account,
                                   ...
                                   new_amount=40,
                                   new_currency=self.chf)
        t3 = Transaction.objects.create(account=self.account,
                                            currency=self.chf,
                                            date=datetime.datetime.today() - datetime.timedelta(days=2),
                                            name="Achat 2 en suisse",
                                            amount=40)
        t1 = Transaction.objects.get(pk=t1.pk)
        t2 = Transaction.objects.get(pk=t2.pk)
        t3 = Transaction.objects.get(pk=t3.pk)
