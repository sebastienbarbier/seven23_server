...
class ApigoalsTest(TransactionTestCase):
    """ Account retrieve """
    ...
    def setUp(self):
        ...
        Goals.objects.create(account=self.account,
                                     blob='Spending')

        Goals.objects.create(account=self.account,
                                     blob='Spending2')

        Goals.objects.create(account=self.account2,
                                     blob='Spending')
    ...
    def test_goals_retrieve(self):
        ...
        self.client.put('/api/v1/goals/' + str(item['id']), item)

        response = self.client.get('/api/v1/goals')
        assert response.json()[0]['blob'] == 'New Blob'

    def test_goals_bulk(self):
        """
            Retrieve data with a user owning an account and being gest in an other one.
        """
        self.client.force_authenticate(user=self.user)
        ...
        self.client.patch('/api/v1/goals', [{'id': edited_item['id'], 'blob': 'Edited blob'}, {'id': edited_item2['id'], 'blob': 'Edited blob'}], format='json')

        response = self.client.get('/api/v1/goals/' + str(edited_item['id']))
        data = response.json()

        assert edited_item['blob'] != 'Edited blob'
        assert data['blob'] == 'Edited blob'

        response = self.client.get('/api/v1/goals/' + str(edited_item2['id']))
        data = response.json()
    ...
    def test_goals_since_last_edited(self):
        """
            Retrieve data with a user owning an account and being gest in an other one.
        """
        self.client.force_authenticate(user=self.user)
        ...
        response = self.client.get('/api/v1/goals?last_edited=%s' % minDate)
        data = response.json()
        assert len(response.json()) == 1